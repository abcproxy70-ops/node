...
echo -e "  ${YELLOW}XanMod + BBRv3 + Очистка + Сетевой стек + Conntrack + Hardening${NC}"
echo ""
sleep 1
...
sysctl -w net.netfilter.nf_conntrack_generic_timeout=120 2>/dev/null || true
...
# Быстрое освобождение FIN_WAIT сокетов (дефолт 60 — слишком долго)
